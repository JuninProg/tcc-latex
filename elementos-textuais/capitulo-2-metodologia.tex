...
A documentação foi implementada com a especificação \emph{OpenAPI} por meio da ferramenta \emph{Swagger} \citeonline{swaggerdocs}, permitindo a geração automática de documentação interativa e a validação das rotas diretamente na \emph{Swagger UI}. Essa integração mantém a documentação sincronizada com o código-fonte e reduz a necessidade de manutenção manual.

O gerenciamento de dependências internas foi realizado com a biblioteca \emph{awilix} \citeonline{awilixdocs}, que implementa o padrão de Injeção de Dependência (\emph{Dependency Injection}). Essa técnica consiste em fornecer a um componente suas dependências externas por meio de abstrações, em vez de criá-las internamente, promovendo baixo acoplamento e facilitando testes \cite{REFERENCIA_DI}.
...
O Painel Administrativo foi desenvolvido como uma \emph{Single Page Application} (SPA), aplicação \emph{web} que carrega uma única página HTML e atualiza seu conteúdo de forma dinâmica, sem recarregamento completo \cite{REFERENCIA_SPA}. Essa abordagem proporciona maior fluidez na interação e reduz o tempo de resposta, sendo adequada para integração contínua com a \emph{API}.

A implementação utilizou \emph{React.js}, biblioteca amplamente empregada para construção de \emph{interfaces} baseadas em componentes reutilizáveis \cite{REFERENCIA_REACT}. A arquitetura de pastas foi adaptada da \emph{API} para atender às necessidades do \emph{frontend}, conforme a Tabela~\ref{tab:pastas_painel}.
...
Para o gerenciamento de rotas, foi utilizada a biblioteca \emph{react-router-dom} \cite{REFERENCIA_ROUTER}. Essa ferramenta permite definir caminhos, associar componentes e implementar navegação programática de forma estruturada no \emph{frontend}.
...
O processo de construção e otimização do Painel Administrativo foi realizado com o \emph{Vite} \cite{REFERENCIA_VITE}. Essa ferramenta de empacotamento oferece inicialização quase instantânea do servidor de desenvolvimento e atualização automática (\emph{hot module replacement}) durante alterações no código.
