...
Com base nesses princípios, o planejamento do Pró-Mamá adotou desde o início uma visão ampla do sistema, mapeando componentes, sua distribuição na infraestrutura e os fluxos de comunicação. Produziu-se uma representação inicial, ainda passível de ajustes, que evidencia as fronteiras entre o núcleo de negócio e os detalhes de implementação, conforme orienta a Arquitetura Limpa de \citeonline{archlimpaunclebob}. Na sequência, essa representação será apresentada e discutida.

\subsection{Representação dos Componentes}

A Figura~\ref{fig:macro-promama} apresenta a visão geral do sistema Pró-Mamá na forma de um esquema conceitual. Essa representação destaca os dois grupos principais de usuários — o administrador e a mãe — e os pontos de interação de cada um com a plataforma. O administrador acessa um painel \emph{web} interno, responsável pela gestão de conteúdo, acompanhamento de métricas e resposta a dúvidas. A mãe, por sua vez, utiliza um aplicativo móvel externo para cadastrar crianças, registrar marcos de desenvolvimento, consultar informações e enviar dúvidas.

No centro da infraestrutura, um serviço de balanceamento de carga (\emph{NGINX Load Balancer}) intermedia as requisições, encaminhando-as para a \emph{API} monolítica, que concentra operações de autenticação, manipulação de dados e envio de notificações. Essa \emph{API} está isolada na rede interna, reforçando o controle de acesso e a segurança dos dados.

A estrutura evidencia, desde essa visão inicial, a separação entre a lógica do domínio (núcleo de regras de negócio presentes na \emph{API}) e elementos externos mais suscetíveis a mudanças, como a interface \emph{web}, o aplicativo móvel e o mecanismo de balanceamento. Esse arranjo reflete a orientação de \citeonline{archlimpaunclebob}, favorecendo a substituição ou evolução de partes periféricas sem impacto direto no núcleo do sistema.

\begin{figure}[H]
\centering
\includegraphics[width=1\textwidth]{imagens/promamamacro.png}
