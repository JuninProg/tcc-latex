...
Com base nesses princípios, o planejamento do Pró-Mamá adotou desde o início uma visão ampla do sistema, mapeando componentes, sua distribuição na infraestrutura e os fluxos de comunicação. Produziu-se uma representação inicial, ainda passível de ajustes, que evidencia as fronteiras entre o núcleo de negócio e a infraestrutura de suporte, conforme orienta a Arquitetura Limpa de \citeonline{archlimpaunclebob}. Na sequência, essa representação será apresentada e discutida.

\subsection{Representação dos Componentes}

A Figura~\ref{fig:macro-promama} apresenta a visão geral do sistema Pró‑Mamá na forma de esquema conceitual. Essa representação destaca os dois grupos principais de usuários — administrador e mãe — e os respectivos pontos de interação com a plataforma.

O administrador acessa um painel \emph{web} interno, utilizado para a gestão de conteúdo, acompanhamento de métricas e resposta a dúvidas. A mãe, por sua vez, interage por meio de um aplicativo móvel externo, no qual é possível cadastrar crianças, registrar marcos de desenvolvimento, consultar informações e enviar questionamentos.

No centro da infraestrutura, um serviço de balanceamento de carga (\emph{NGINX Load Balancer}) recebe as requisições e as direciona para a \emph{API} monolítica, responsável por autenticação, manipulação de dados e envio de notificações. A \emph{API} está isolada na rede interna, o que reforça o controle de acesso e a segurança das informações.

A estrutura proposta evidencia, desde esta visão inicial, a separação entre a lógica do domínio, concentrada na \emph{API}, e componentes técnicos suscetíveis a troca (camadas externas: \emph{interface} \emph{web}, aplicativo móvel e mecanismo de balanceamento). Essa separação segue o princípio de \citeonline{archlimpaunclebob}, segundo o qual aspectos tecnológicos periféricos devem permanecer ao redor do núcleo para que possam evoluir sem impacto direto nas regras centrais do sistema.

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{imagens/promamamacrov2.png}
\caption{Visão geral dos componentes e interações do sistema Pró-Mamá.}
