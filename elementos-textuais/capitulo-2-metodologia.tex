...
O presente estudo caracteriza-se como pesquisa aplicada, de natureza tecnológica e abordagem mista \cite{siena2024metodologia}. Foram combinadas técnicas qualitativas e quantitativas, como levantamento documental e estudo de caso único, com foco na plataforma Pró-Mamá e seus serviços \emph{web} (\emph{API} e painel administrativo). O aplicativo móvel, embora relacionado, não constitui objeto central por ser resultado de outro trabalho acadêmico.
...
\section{Uso de ferramentas de inteligência artificial na produção textual}

Durante a elaboração deste trabalho foram utilizadas ferramentas de inteligência artificial generativa como apoio auxiliar à produção textual. Seu uso restringiu-se à revisão gramatical e ortográfica, identificação de repetições, melhorias de clareza textual e organização preliminar de trechos do documento.

A utilização dessas ferramentas ocorreu sob uma abordagem centrada no ser humano, conforme recomendado pela \citeonline{unesco2023genai}. Dessa forma, os recursos de IA foram empregados apenas como instrumentos de apoio, sem substituir a capacidade crítica, a interpretação das informações ou a responsabilidade do pesquisador sobre o conteúdo produzido.
...
A engenharia de requisitos é a atividade de compreender e definir os serviços requisitados do sistema e identificar restrições relativas à sua operação e desenvolvimento, sendo considerada um estágio crítico da engenharia de \emph{software}, pois ``erros nessa fase inevitavelmente geram problemas no projeto e na implementação do sistema'' \cite[p.~24]{sommerville2011engenharia}. Nessa etapa, a análise transforma necessidades dos usuários e restrições de negócio em especificações claras e compreensíveis.
...
\hline
\end{tabular}
\caption[RN01 -- Categorização de conteúdo por faixa etária]{Regra de Negócio 1}
\label{tab:rn01}
\end{table}
...
\hline
\end{tabular}
\caption[RN02 -- Proteção de dados pessoais]{Regra de Negócio 2}
\label{tab:rn02}
\end{table}
...
\hline
\end{tabular}
\caption[RN03 -- Respostas a dúvidas de mães]{Regra de Negócio 3}
\label{tab:rn03}
\end{table}
...
\hline
\end{tabular}
\caption[RN04 -- Isolamento de dados por município]{Regra de Negócio 4}
\label{tab:rn04}
\end{table}
...
\hline
\end{tabular}
\caption[RN05 -- Métricas de uso]{Regra de Negócio 5}
\label{tab:rn05}
\end{table}
...
\hline
\end{tabular}
\caption[RN06 -- Notificações personalizadas]{Regra de Negócio 6}
\label{tab:rn06}
\end{table}
...
Para a \emph{API} e a infraestrutura \emph{web}, consideraram-se requisitos os elementos necessários para garantir o funcionamento adequado e seguro da plataforma. Isso inclui aspectos como autenticação, autorização, comunicação entre serviços e armazenamento de dados e arquivos. Para identificar esses requisitos, utilizou-se o sufixo ``API'' ao final do ID, facilitando a distinção entre os sistemas.
...
\hline
\end{tabular}
\caption[RFAPI01 -- Persistência de dados]{Requisito Funcional 1}
\label{tab:rfapi01}
\end{table}
...
\hline
\end{tabular}
\caption[RFAPI02 -- Autenticação de usuários]{Requisito Funcional 2}
\label{tab:rfapi02}
\end{table}
...
\hline
\end{tabular}
\caption[RFAPI03 -- Autorização de usuários]{Requisito Funcional 3}
\label{tab:rfapi03}
\end{table}
...
\hline
\end{tabular}
\caption[RFAPI04 -- Armazenamento de arquivos]{Requisito Funcional 4}
\label{tab:rfapi04}
\end{table}
...
\hline
\end{tabular}
\caption[RFAPI05 -- Agendamento de notificações]{Requisito Funcional 5}
\label{tab:rfapi05}
\end{table}
...
\hline
\end{tabular}
\caption[RNFAPI01 -- Privacidade dos dados dos usuários]{Requisito Não Funcional 1}
\label{tab:rnfapi01}
\end{table}
...
\hline
\end{tabular}
\caption[RNFAPI02 -- Uso de HTTPS e certificado digital]{Requisito Não Funcional 2}
\label{tab:rnfapi02}
\end{table}
...
\hline
\end{tabular}
\caption[RNFAPI03 -- Escalabilidade para diferentes servidores]{Requisito Não Funcional 3}
\label{tab:rnfapi03}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL01 -- Cadastrar bairro]{Requisito Funcional 1}
\label{tab:rfpanel01}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL02 -- Cadastrar posto de saúde]{Requisito Funcional 2}
\label{tab:rfpanel02}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL03 -- Cadastrar informação]{Requisito Funcional 3}
\label{tab:rfpanel03}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL04 -- Cadastrar notificação]{Requisito Funcional 4}
\label{tab:rfpanel04}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL05 -- Cadastrar dúvida frequente]{Requisito Funcional 5}
\label{tab:rfpanel05}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL06 -- Cadastrar termos de uso]{Requisito Funcional 6}
\label{tab:rfpanel06}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL07 -- Responder dúvida do usuário]{Requisito Funcional 7}
\label{tab:rfpanel07}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL08 -- Alterar senha]{Requisito Funcional 8}
\label{tab:rfpanel08}
\end{table}
...
\hline
\textbf{Descrição} & O sistema deve enviar uma notificação ao administrativo do Painel quando uma nova dúvida de usuário chegar. Quando o administrador responder, uma mensagem deve ser encaminhada para o usuário que fez a pergunta. \\
\hline
...
\hline
\end{tabular}
\caption[RFPANEL09 -- Enviar e-mail na interação do Fale Conosco]{Requisito Funcional 9}
\label{tab:rfpanel09}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL10 -- Pré-visualizar informação]{Requisito Funcional 10}
\label{tab:rfpanel10}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL11 -- Remoção de conta]{Requisito Funcional 11}
\label{tab:rfpanel11}
\end{table}
...
\hline
\end{tabular}
\caption[RFPANEL12 -- Exportação de relatórios]{Requisito Funcional 12}
\label{tab:rfpanel12}
\end{table}
...
\hline
\end{tabular}
\caption[RNFPANEL01 -- Desenvolvimento em plataforma web]{Requisito Não Funcional 1}
...
\hline
\end{tabular}
\caption[RNFPANEL02 -- Acesso pela Internet]{Requisito Não Funcional 2}
...
\hline
\end{tabular}
\caption[RNPANEL03 -- Fornecimento de API para consumo das informações]{Requisito Não Funcional 3}
...
\hline
\end{tabular}
\caption[RNPANEL04 -- Uso do banco MySQL]{Requisito Não Funcional 4}
...
\hline
\end{tabular}
\caption[RNFPANEL05 -- Alta disponibilidade]{Requisito Não Funcional 5}
...
\hline
\end{tabular}
\caption[RNFPANEL06 -- Design responsivo]{Requisito Não Funcional 6}
...
\hline
\end{tabular}
\caption[RNPANEL07 -- Acesso por navegador web]{Requisito Não Funcional 7}
...
Com base nesse diagnóstico, a adoção dos princípios da arquitetura limpa de \citeonline{archlimpaunclebob} foi estratégica para assegurar flexibilidade e sustentabilidade. A distinção entre o núcleo das regras e componentes voláteis permite atualizar ou substituir tecnologias e fornecedores sem interferir no funcionamento central da solução.
...
A organização dos arquivos está alinhada ao conceito de “Arquitetura Gritante” descrito por \citeonline{archlimpaunclebob}, segundo o qual a estrutura do código deve refletir o domínio do negócio, permitindo rápida identificação de seu propósito apenas ao analisar sua organização.
...
A implementação utilizou \emph{React.js}, biblioteca amplamente empregada para construção de \emph{interfaces} baseadas em componentes reutilizáveis \cite{react}. A arquitetura de pastas foi adaptada da \emph{API} para atender às necessidades do \emph{front-end}, conforme a Tabela~\ref{tab:pastas_painel}.
...
\texttt{components} & Componentes visuais reutilizáveis da \emph{interface} (\emph{UI}). \\ \hline
...
\texttt{infra} & Integrações com serviços externos e recursos de infraestrutura do \emph{front-end}. \\ \hline
...
Para o gerenciamento de rotas, foi utilizada a biblioteca \emph{react-router-dom} \cite{reactrouter}. Essa ferramenta permite definir caminhos, associar componentes e implementar navegação programática de forma estruturada no \emph{front-end}.
