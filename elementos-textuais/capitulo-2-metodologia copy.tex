...
% Backend Node.js / Express / Awilix injeção de dependência / Typescript
% NGINX / HTTPS
% Frontend React.js / Typescript / Componentes
% POSTGRES
% DOCKER / DOCKER COMPOSE
% CLEAN CODE, SOLID, DDD
% SWAGGER / OPENAPI 3.0

% Contextualizar o que é desenvolvimento web e porque faz sentido dado o que o Pró-Mamá é, exemplificar com o aplicativo que se comunica com a aplicação web, bem como o painel administrativo faz a mesma comunicação
% Criar subseções para o arquitetura, backend, frontend, infraestrutura e devops
% Para a subseção de arquitetura, discutir a estrutura geral do sistema, incluindo a comunicação entre os componentes e a integração com serviços externos, boas práticas clean code
% Para a subseção de backend, abordar a implementação do servidor, gerenciamento de rotas e controle de acesso, além de discutir a injeção de dependências e a estrutura do código
% Para a subseção do frontend, explorar a construção da interface do usuário, a gestão de estado e a comunicação com a API, além de discutir a importância de componentes reutilizáveis
% Para a subseção da infraestrutura, discutir a configuração do ambiente de desenvolvimento, a utilização de containers com Docker e a orquestração de serviços, Nginx como load balancer
% Para a subseção de devops, discutir sobre o github com github actions para integração e entrega contínua, com deploy automatizado